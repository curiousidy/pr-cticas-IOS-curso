...
class Animal{
    var raza:String
    var nombre:String
    init(withRaza: String, name: String){
        self.raza = withRaza
        self.nombre = name
    }
}
...
var perro = Perro(withRaza: "Golden Retriever", name: "Lola")
...
//Cambia la clase Igualdad a struct y piensa que sucede no son iguales porque son diferntes objetos y apuntan a diferentes direcciones de memoria
...
//en el struct si son iguales porque es un dato primitivo y como si se comparasen 2 string.
...
struct agenda {
    ...
    mutating func search(str:String)-> Contact{
        for c in contactos{
