...
func letrasRepetidas (_ word: String) -> Bool {
    var iterletters:String = ""
    for letters in word{
        if (iterletters.contains(letters)){
            return true
        }
        iterletters.insert(letters, at: iterletters.endIndex)
    }
        return false
}
...
func palindromo (_ word: String) -> Bool {
...
func eliminarRepetidas (str: String) -> String {
 ...
 assert(eliminarRepetidas(str: "hello") == "helo", "SI REVIENTO AQUI")
...
func getWrongAnswers(n: Int, c:String) -> String {
  // Write your code here
    ...
    for i in c{
        if i == "A"{
            respuestas.insert("B", at: respuestas.endIndex)
        }
        else{
            respuestas.insert("A", at: respuestas.endIndex)
        }
    }
    return respuestas
}
